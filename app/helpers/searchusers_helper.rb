...
module SearchusersHelper
  ...
  def display_basic_search_results(objects)
    objects.each_with_object('') do |object, string|
      string << content_tag(:div, content_tag(:div, content_tag(:div, content_tag(:div, tag("img", src: object.avatar.url(:full), class: "profile-avatar"), class: "row") + display_basic_search_results_row(object), class: ["large-3", "small-3", "columns"], align: "center") + content_tag(:div, "",class: ["large-1", "small-1", "columns"])))#code        
    end
  end
